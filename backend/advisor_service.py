...
def get_financial_advice_stream(user_prompt, financial_data):
    ...
    try:
        ...
        system_prompt = _sanitize_for_ai(f"""You are the PerfinLab AI Advisor — a sharp, data-driven financial analyst.
...
def generate_overview(financial_data, brief_type="morning"):
    ...
    system_prompt = _sanitize_for_ai(f"""You are the PerfinLab AI Analyst preparing {persona}.
...
def generate_market_intelligence(financial_data):
    ...
    system_prompt = _sanitize_for_ai(f"""You are the PerfinLab AI Analyst generating a Market Pulse update for a dark-themed dashboard panel.
